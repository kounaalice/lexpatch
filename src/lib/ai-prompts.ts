...
## プロジェクト管理（/projects）
- 複数の改正案をまとめてプロジェクト管理
- タスク管理（チェックリスト形式）+ テンプレート4種
- ガントチャートでタスク時系列表示
- 議事録メモで会議記録管理
- プロジェクトカレンダーでスケジュール管理
- R2ファイル添付（PDF・画像等）
- メンバー招待で共同編集
...
- フォロー法令の改正検知 → メール通知
- 通知設定で5カテゴリ（法令改正/コミュニティ/プロジェクト/フォロワー/お知らせ）管理
- ダッシュボードに通知バッジ
...
## ゲーミングモード（/settings → テーマ）
- 設定ページでゲーミングモードON/OFF
- XPバー・パーティクルエフェクト・称号
- 閲覧XP（法令を読むと獲得）+ 活動ポイント（編集・投稿で獲得）
- 条文カードゲーム（/cards）
- 6テーマ: ライト/ダーク/ゴシック/クラシック/ゆめかわ/みずいろ

## ダッシュボード（/dashboard）
- ログイン後のホーム画面
- ウィジェット: 統計/タスク/施行予定/ゲーミング/ブックマーク/フォロー/履歴/ノート/AI
...
function pageContextHint(currentPath: string): string {
  ...
  if (currentPath.startsWith("/projects")) {
    return "ユーザーはプロジェクトページにいます。タスク管理・ガントチャート・議事録・カレンダー・ファイル添付が利用できます。";
  }
  if (currentPath.startsWith("/communities")) {
    return "ユーザーはコミュニティページにいます。パッチ共有・メンバーフォローが利用できます。";
  }
  ...
  if (currentPath.startsWith("/cards")) {
    return "ユーザーは条文カードゲームページにいます。ゲーミングモードの機能です。";
  }
