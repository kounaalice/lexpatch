...
// ISR: 30日キャッシュ（カテゴリ構成は安定、デプロイ時に再生成。CDN エッジキャッシュ最大化）
...
export async function generateMetadata({
  ...
  if (!cat) return {};
  const title = `${cat.label}の法令一覧`;
  const desc = `${cat.label}に関する法令・法律の一覧。条文の閲覧・検索・改正案作成が可能。LexCard — 法令アクセス支援システム`;
  return {
    title,
    description: desc,
    openGraph: { title: `${cat.label} | LexCard`, description: desc },
  };
}
      ...
      {isJorei && prefecture && (
        <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "1.5rem 2rem 0" }}>
          ...
          >
            ...
            >
              {prefecture.name}
              の公式サイトから例規集・条例データベースにアクセスできます。条例の正確な内容は各自治体の公式サイトをご確認ください。
            </p>
            <a
              href={prefecture.topUrl}
              target="_blank"
                  ...
                  {prefs.map((p) => (
                    <Link
                      ...
                      style={{
                        ...
                        fontSize: "0.78rem",
                        color:
                          p.slug === slug ? "var(--color-accent)" : "var(--color-text-secondary)",
